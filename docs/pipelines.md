# Pipelines + Secrets（Stage 1）

本文是武陵 DevOps Pipelines（CI/CD）与 Secrets 子系统的设计与契约文档。它同时是
后端（Go 控制面）、Runner 客户端（Rust，`runners/runner-clients`）和前端三方共同遵守
的接口规范。

> 状态：Stage 1 实装中。语法风格对齐 **GitHub Actions** 的一个子集；Runner 与服务端走
> **HTTP 长轮询 + 令牌**；Secrets 用 **AES-256-GCM**；任务队列落在 **Postgres**。
> Kubernetes 后端**暂不实现**——按计划等自研 K8s 发行版（见“路线图”）。
...
| 方法 & 路径 | 作用 |
|----|----|
| `POST /api/v1/runner/register` | 用**注册令牌**（org maintainer 在 UI 生成，短期）换取持久 runner token + runner id；上报 labels/tier/容量。 |
...
- `pools[]`：每个池绑定一个 `provider` + `tier`，带 `labels`、`min`、`max`，以及 provider 专属字段；
  云凭证以 `credentials_secret`（org Secret 名）引用。
...
3. **扩容**：匹配的 pool 若未达 `max`，调用 `Provider.Launch(spec)` 拉起实例，并通过 user-data /
   cloud-init 注入「服务端地址 + 一次性注册令牌 + labels/tier」，新机自举后向控制面注册。
...
- step 级 `uses` 第三方 action 生态、matrix、可复用 workflow、环境/审批门禁 → Stage 2+。
